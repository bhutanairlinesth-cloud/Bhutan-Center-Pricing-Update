...
import {
  ...
  Plane, Plus, ReceiptText, RefreshCw, Save, Settings2, ShieldCheck, Trash2, UserPlus, Users, WandSparkles,
} from 'lucide-react';
  ...
  return <div className="admin-stack pricing-control-center">
    ...
    <section className="pricing-global-section">
      ...
      <div className="settings-grid pricing-clean-grid">
        ...
        <section className="settings-card"><div className="settings-card-title"><span><BadgePercent/></span><div><h3>{t('margins')}</h3><p>THB / person</p></div></div><NumberField label={t('retailMargin')} value={form.marginTHB} onChange={(v) => change('marginTHB', v)}/><NumberField label={t('agentMargin')} value={form.agentMarginTHB ?? 3000} onChange={(v) => change('agentMarginTHB', v)}/><NumberField label={language === 'th' ? 'Margin โรงแรม 5 ดาว / ท่าน' : '5-star hotel margin / pax'} value={form.hotel5StarMarginTHB ?? 10000} onChange={(v) => change('hotel5StarMarginTHB', v)}/><div className="calculated-note"><HotelIcon/> {language === 'th' ? 'เมื่อเลือก 5 Stars ระบบจะใช้ Margin นี้แทน Margin ปกติทั้ง Retail และ Agent' : 'When 5 Stars is selected, this margin overrides the normal Retail/Agent margin.'}</div></section>
