-- Bhutan Center Pricing: database, seed data and RLS
...
create policy "hotels read authenticated" on public.hotels for select to authenticated using (true);
...
create policy "packages read authenticated" on public.tour_packages for select to authenticated using (true);
...
-- After creating the first user in Authentication > Users, make that account admin:
-- insert into public.profiles (id,name,email,role)
-- select id,'Bhutan Airlines TH',email,'admin'::public.user_role
