...
function NumberField({ label, value, onChange, step = '1', suffix = 'THB' }: { label: string; value: number; onChange: (value: number) => void; step?: string; suffix?: string }) {
  return <label className="field number-field"><span>{label}</span><div><input type="number" min="0" step={step} value={value} onChange={(event) => onChange(Number(event.target.value))}/><em>{suffix}</em></div></label>;
}
