...
function friendlyAuthError(message?: string) {
  ...
  if (/email not confirmed/i.test(message)) return 'บัญชียังไม่ได้ยืนยันอีเมล กรุณายืนยันใน Supabase';
...
export async function fetchProfile(userId: string): Promise<any> {
  ...
  if (!data) {
    throw new Error('เข้าสู่ระบบสำเร็จ แต่ยังไม่มี Profile กรุณารันไฟล์ supabase/production_setup.sql ใน SQL Editor');
  }
  return data;
}
