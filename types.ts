...
import { 
  ...
  RefreshCw, Calculator, Database, CloudOff
} from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-brand-cream flex flex-col font-sans text-gray-800">
      ...
      {/* HEADER BAR */}
      <header className="bg-white/95 backdrop-blur-xl border-b border-emerald-950/5 h-16 shrink-0 flex items-center justify-between px-4 md:px-6 z-30 sticky top-0 premium-shadow">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="leading-tight">
              <span className="font-serif font-bold text-base md:text-lg text-brand-emerald tracking-tight block">
                Bhutan Center
              </span>
              <span className="hidden sm:block text-[10px] uppercase tracking-[0.18em] text-brand-gold-dark font-bold">Pricing & Quotation System</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <div className="hidden lg:flex items-center gap-1.5 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-[10px] font-bold text-amber-800" title="ข้อมูลยังถูกจัดเก็บเฉพาะในเบราว์เซอร์เครื่องนี้">
      ...
      {isSidebarOpen && (
        <button
          ...
          className="fixed inset-0 top-16 z-30 bg-slate-950/30 backdrop-blur-[2px] md:hidden"
        />
      )}
          ...
          <div className="space-y-3">
            ...
            <div className="bg-slate-50 rounded-xl p-3 text-center border border-slate-100">
              <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider font-mono">
                Bhutan Center • Version 2.5
              </p>
              <p className="text-[9px] text-gray-400 mt-0.5">
                ข้อมูลเวอร์ชันทดลองเก็บในเบราว์เซอร์
              </p>
            </div>
          </div>
        </aside>
